...
export default function PracticePage() {
  ...
  const [notebook, setNotebook] = useState<Caderno | null>(null);
  ...
  useEffect(() => {
    ...
    if (notebookToEdit) {
      ...
      if (notebookToEdit.filters)
        setFilters(normalizeFilters(notebookToEdit.filters));
  ...
  const handleSaveNotebook = async () => {
    console.log("[PracticePage] handleSaveNotebook iniciando...", {
      userId: user?.id,
    ...
    if (!user?.id || !newNotebookName.trim()) {
      console.log(
        "[PracticePage] handleSaveNotebook: userId ou nome vazio, abortando"
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        "[PracticePage] handleSaveNotebook: prepIdToUse:",
        prepIdToUse,
        "isSubscriber:",
        isSubscriber
      );
      ...
      if (prepIdToUse && !isSubscriber) {
        ...
        console.log(
          "[PracticePage] handleSaveNotebook: resultado bateria:",
          batteryResult
        );
        ...
        console.log(
          "[PracticePage] handleSaveNotebook: pulando bateria (assinante ou sem prep)"
        );
      }
      ...
      console.log(
        "[PracticePage] handleSaveNotebook: notebook criado:",
        newNotebook?.id
      );
      ...
      if (newNotebook) {
        ...
        console.log(
          "[PracticePage] handleSaveNotebook: recarregando notebooks..."
        );
        await loadNotebooks();
        ...
        console.log(
          "[PracticePage] handleSaveNotebook: notebook retornou null"
        );
  ...
  // start with notebook id from notebooks page
  useEffect(() => {
    const loadNotebook = async () => {
      if (!user?.id) return;

      try {
        const notebook = await getNotebookFromId(notebookId, user.id);
    ...
    try {
      ...
      if (usingMockData) {
        ...
        if (activeFilters.materia.length > 0) {
          filtered = filtered.filter((q) =>
            activeFilters.materia.includes(q.materia)
          );
        }
        if (activeFilters.banca.length > 0) {
          filtered = filtered.filter((q) =>
            activeFilters.banca.includes(q.banca)
          );
        }
        if (activeFilters.ano.length > 0) {
          filtered = filtered.filter((q) =>
            activeFilters.ano.includes(String(q.ano))
          );
        }
        ...
        const dbQuestions = await fetchQuestions({
          materias:
            activeFilters.materia.length > 0
              ? activeFilters.materia
              : undefined,
          assuntos:
            activeFilters.assunto.length > 0
              ? activeFilters.assunto
              : undefined,
          bancas:
            activeFilters.banca.length > 0 ? activeFilters.banca : undefined,
          orgaos:
            activeFilters.orgao.length > 0 ? activeFilters.orgao : undefined,
          cargos:
            activeFilters.cargo.length > 0 ? activeFilters.cargo : undefined,
          anos:
            activeFilters.ano.length > 0
              ? activeFilters.ano.map(Number)
              : undefined,
          escolaridade:
            activeFilters.escolaridade.length > 0
          ...
          modalidade:
            activeFilters.modalidade.length > 0
              ...
              : undefined,
          dificuldade:
            activeFilters.dificuldade.length > 0
              ? activeFilters.dificuldade
              : undefined,
          apenasRevisadas: activeToggleFilters.apenasRevisadas || undefined,
          apenasComComentario:
            activeToggleFilters.apenasComComentario || undefined,
          limit: isTrailMode ? 500 : questionCount, // Modo trilha: buscar até 500 questões
          shuffle: true,
        });
        console.log("[PracticePage] Questões recebidas:", dbQuestions);
