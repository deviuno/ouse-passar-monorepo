...
const CommentItem: React.FC<{
  ...
  return (
    <div className={`flex flex-col ${isReply ? 'ml-8 pl-4 border-l-2 border-gray-700' : ''}`}>
      <div className="flex items-start space-x-3 mb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            <span className="font-bold text-sm text-gray-300 truncate">{userName}</span>
            <span className="text-xs text-gray-600 shrink-0">{getRelativeTime(comment.created_at)}</span>
          </div>
          <p className="text-sm text-gray-400 leading-relaxed break-words">{comment.content}</p>
          ...
          <div className="flex items-center space-x-4 mt-2">
            ...
            >
              <ThumbsUp size={14} className={comment.user_vote === 'like' ? 'fill-[#FFB800]' : ''} />
            ...
            {!isReply && userId && (
              <button
                ...
                className="text-xs text-gray-600 hover:text-white font-medium flex items-center"
              >
      ...
      {comment.replies && comment.replies.length > 0 && (
        <div className="mt-2">
          <button
            ...
            className="flex items-center text-xs text-[#FFB800] hover:text-[#FFC933] mb-2 ml-11"
          >
...
const CommentsSection: React.FC<CommentsSectionProps> = ({ questionId, userId, onShowToast }) => {
  ...
  return (
    <div className="mt-6">
      ...
      <div className="mb-6">
        {replyingTo && (
          <div className="flex items-center justify-between bg-gray-800/50 px-3 py-2 rounded-t-lg border border-b-0 border-gray-700">
            <span className="text-xs text-gray-400">
              Respondendo a <span className="text-[#FFB800]">{getReplyingToName()}</span>
            </span>
        ...
        <div className={`flex items-center bg-[#252525] border border-gray-700 ${replyingTo ? 'rounded-b-lg' : 'rounded-lg'}`}>
          <input
            type="text"
            value={newComment}
            ...
            className="flex-1 bg-transparent px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none disabled:opacity-50"
          />
          <button
            onClick={handleSubmit}
            disabled={!userId || !newComment.trim() || isSubmitting}
            className="p-3 text-[#FFB800] hover:text-[#FFC933] disabled:text-gray-600 disabled:cursor-not-allowed transition-colors"
          >
            <Send size={18} />
          </button>
        </div>
